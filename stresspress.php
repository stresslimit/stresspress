...
// Limit post revisions, todo: this should go in wp-config
...
// add_action('admin_head', 'rm_post_custom_fields');
function rm_post_custom_fields() {
	// pages
...
function setup_postmeta_all( $post ) {
	global $wpdb;
	$post_id = $post->ID;
	$sql = "
...
add_action('login_head', 'admin_css_styles');
add_action('admin_head', 'admin_css_styles');
...
function admin_footer_brand() {
	echo date("Y").' <a href="http://stresslimitdesign.com">stresslimitdesign</a>';
} 
add_filter('admin_footer_text', 'admin_footer_brand');
